...
\begin{document}
\newcommand{\scatterfile}{scatter_*.quantile.csv.table.csv}
\begin{tikzpicture}
\begin{loglogaxis}[
    xlabel={\small CPU time for SMPMC (\second)},
    ylabel={\small CPU time for *.quantile.csv (\second)},
    ...
    % Ensure scatter_*.quantile.csv.table.csv has 'sat' and 'unsat' in the meta column (index 2)
