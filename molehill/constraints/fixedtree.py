...
class FixedDecisionTree(Constraint):
    def __init__(self):
        super().__init__()
    ...
    def show_result(self, model, _solver, **args):
        ...
        first_variable_name = str(self.variables[0])
        property_names = get_property_names(first_variable_name)

        num_nodes = 2**self.tree_depth - 1
        max_action_size = max([x.size() for x in self.variables])
        leaf_values = [
            z3.BitVec(f"leaf_{i}", max_action_size) for i in range(num_nodes + 1)
        ]
        ...
        def build_anytree(node_index, depth):
            if node_index >= 2**self.tree_depth - 1:
