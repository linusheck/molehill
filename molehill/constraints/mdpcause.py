...
class MDPCause(Constraint):
    ...
    def build_constraint(self, function, variables, variables_in_ranges, **args):
        ...
        with open(os.path.join(args["project_path"], "values.csv"), "r") as f:
            # first line is header (variable names)
        ...
        # look into transition matrix of quotient to check at which states there is only one action available
        # these states should be thrown out of the variable_value_pairs
        ...
        for state in range(quotient.quotient_mdp.nr_states):
            ...
            if len(choice_to_hole_options[first_row]) == 0:
                print(f"State {state} has no choices, skipping.")
            else:
                variable_value_pairs.append(variable_value_pairs_orig[state])



        print(variable_value_pairs)

        # Use variable_value_pairs instead of parsing property names/values
        # variable_value_pairs are sorted the same as policy_vars
        ...
        for i, variable in enumerate(policy_vars):
            ...
            # Convert dict values to list in consistent order, and to Z3 Reals
            ...
            print("label range", label_range, labels)
            if label_range == labels:
                ...
                for index, label_index in enumerate(label_indices):
                    if self.args.relax:
                        # relax this a bit: if x is out of range, we allow *
