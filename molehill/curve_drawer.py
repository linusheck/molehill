...
def draw_curve(num_bits, variables, s, p, model=None, gif=True):
    ...
    pixels = image.load()
    new_assertions = p.image_assertions
    ...
    for j in range(0, N + 1):
        ...
        color = (255, 0, 0, 255)
        if i < len(p.reasons) + 1:
            if i == 0:
                color = (245, 245, 220, 255)
            else:
                reason = p.reasons[i - 1]
                for j, r in enumerate(reasons):
                    if r in reason:
                        color = high_contrast_colors[j - 1]
                        break
        ...
        if gif:
            ...
            copied_image = copied_image.resize(
                (size * 10, size * 10), Image.Resampling.NEAREST
            )
            ...
            if i == 0:
                draw.text("Z3")
            elif i < len(p.reasons) + 1:
                draw.text(
                    (0, size * 10),
                    p.reasons[i - 1]
                    + f"\n{len(possible_models)} models left\n"
                    + str(new_assertions[i]),
                    font=font,
                    fill="black",
                )
                ...
                draw.text(
                    (0, size * 10),
                    "model found:"
                    + "\n".join([f"{var}={model.eval(var)}" for var in variables]),
                    font=font,
                    fill="black",
                )
    ...
    if gif:
        images[0].save(
            "image.gif",
            save_all=True,
            append_images=images[1:] + [images[-1].copy()] * 10,
            duration=100,
            loop=0,
        )
