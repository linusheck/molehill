...
\begin{document}
\newcommand{\scatterfile}{scatter_SMT(LRA).table.csv}
\begin{tikzpicture}
\begin{loglogaxis}[
    xlabel=CPU time for SMPMC (\second),
    ylabel=CPU time for SMT(LRA) (\second),
    xmin=0.1, xmax=50000,
    ymin=0.1, ymax=50000,
    domain=0.1:10001,
    ...
    % Ensure scatter_SMT(LRA).table.csv has 'sat' and 'unsat' in the meta column (index 2)
